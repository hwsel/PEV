...
class StreamerFactory(
    ...
    fun build(): IStreamer {
        ...
        val endpoint = streamer as CameraSrtLiveStreamer
        endpoint.latency = 100

        MediaCodecList(MediaCodecList.REGULAR_CODECS).codecInfos.forEach {
            if (it.isEncoder) {
                Log.i("TEST", "${it.name}, ${it.canonicalName}, ${it.isHardwareAccelerated}, ${it.isSoftwareOnly}")
            }
        }

        Log.i("TEST", "${MediaFormat.MIMETYPE_VIDEO_AVC}")

        val videoConfig = VideoConfig(
            mimeType = MediaFormat.MIMETYPE_VIDEO_AVC,
//            mimeType = configuration.video.encoder,
            startBitrate = configuration.video.bitrate * 400, // to b/s
